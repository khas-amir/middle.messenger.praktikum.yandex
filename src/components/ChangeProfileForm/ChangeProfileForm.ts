    ...
    public componentDidUpdate(oldProps: Props, newProps: Props) {
        if (!isEqual(oldProps, newProps)) {
            this.children.InputEmail.setProps({
                value: newProps.profile.email,
            });
            this.children.InputLogin.setProps({
                value: newProps.profile.login,
            });
            this.children.InputFirstName.setProps({
                value: newProps.profile.first_name,
            });
            this.children.InputSecondName.setProps({
                value: newProps.profile.second_name,
            });
            this.children.InputChatLogin.setProps({
                value: newProps.profile.display_name,
            });
            this.children.InputPhone.setProps({
                value: newProps.profile.phone,
            });
        }
        return true;
    }
        ...
        const onBlur = (e: FocusEvent) => {
            ...
            validator(
                ...
                () => {
                    ...
                    errors[target.name] = '';
                    this.setProps({ errors });
                    this.getComponent(target.name).setProps({
                        value: target.value,
                    });
                }
            );
